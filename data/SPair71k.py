...
pair_ann_path = config.SPair.ROOT_DIR + "/PairAnnotation"
layout_path = config.SPair.ROOT_DIR + "/Layout"
image_path = config.SPair.ROOT_DIR + "/JPEGImages"
dataset_size = config.SPair.size

sets_translation_dict = dict(train="trn", test="test")
difficulty_params_dict = dict(
    trn=config.TRAIN.difficulty_params, val=config.EVAL.difficulty_params,
    test=config.EVAL.difficulty_params
)
...
class SPair71k:
    def __init__(self, sets, obj_resize):
        ...
        self.combine_classes = config.combine_classes
