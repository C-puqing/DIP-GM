...
anno_path = config.VOC2011.KPT_ANNO_DIR
img_path = config.VOC2011.ROOT_DIR + "JPEGImages"
ori_anno_path = config.VOC2011.ROOT_DIR + "Annotations"
set_path = config.VOC2011.SET_SPLIT
...
class PascalVOC:
    def __init__(self, sets, obj_resize):
        ...
        """
        self.classes = config.VOC2011.CLASSES
        self.kpt_len = [len(KPT_NAMES[_]) for _ in config.VOC2011.CLASSES]
    ...
    def filter_list(self):
        ...
        for cls_id in range(len(self.classes)):
            ...
            for xml_name in self.xml_list[cls_id]:
                ...
                if config.exclude_willow_classes:
                    if (
                            self.sets == "train"
