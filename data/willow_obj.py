...
class WillowObject(BaseDataset):
    def __init__(self, sets, obj_resize):
        ...
        self.classes = config.WILLOW.CLASSES
        self.kpt_len = [config.WILLOW.KPT_LEN for _ in config.WILLOW.CLASSES]

        self.root_path = Path(config.WILLOW.ROOT_DIR)
        self.obj_resize = obj_resize

        assert sets == "train" or "test", "No match found for dataset {}".format(sets)
        self.split_offset = config.WILLOW.TRAIN_OFFSET
        self.train_len = config.WILLOW.TRAIN_NUM
        self.sets = sets
