...
class GMDataset(Dataset):
    ...
    def set_train_sampling(self, train_sampling):
    ...
    def __getitem__(self, idx):
        sampling_strategy = config.DATASET_SETTING.train_sampling if self.ds.sets == "train" \
            else config.DATASET_SETTING.eval_sampling
        ...
        anno_list, perm_mat_list = self.ds.get_k_samples(idx, k=self.num_graphs_in_matching_instance, cls=self.cls,
        ...
        if imgs[0] is not None:
            trans = transforms.Compose(
                [transforms.ToTensor(), transforms.Normalize(config.NORM_MEANS, config.NORM_STD)])
